...
object Analysis {
  def main(args: Array[String]) {
    ...
    val tweetsfile = sqlContext.jsonFile("C:\\Python35\\singer.json  ")



    tweetsfile.registerTempTable("querytable1")
    val query = sqlContext.sql("select user.name, count(user.followers_count) as followersCount from querytable1 group by user.name order by followersCount desc limit 100")
    ...
    val query = sqlContext.sql("SELECT device , count(*) as cnt FROM querytable1 WHERE user.verified OR NOT user.verified GROUP BY device HAVING device != '' ORDER BY cnt DESC")
    ...
    query.show();
          ...
          .map(word => (word, 1))
        .reduceByKey(_ + _)
    counts.saveAsTextFile("C:\\Python\\test.txt")
